...
初版（只看前 1000 条时）的结论是"照提"。全量扫描后不再成立：

- 与 `Luaphes` 那条的差异**仍然真实**（它缺进行中、它的琥珀态在当前版本不生效）。
- **但那个差异不足以构成第 5 条收录。** 因为 `bf185003/dsh-favicon-status` 与
  `waknow/dsh-web-icon-indicator` 已经把「favicon 随会话状态变化、含进行中、带动画或配置」做掉了，
  而我们相对它们的优势只剩"标题里带状态词"这一条文案层面的差别。
- 收录评审明确有一条是「**是否与已有条目重复**」。带着这个事实去提，被判重复的概率不低。

**建议：只发 npm，不提收录。** 包照样能被任何人 `dsh plugin --profile web add @climber47/dsh-tab-status` 安装，
只是不进列表、缺少被发现入口 —— 与"提了大概被关"相比，这是更省事也更体面的选择。

若仍要提，唯一站得住的定位是**把卖点收窄到"标签标题里的可读状态词"**（这是唯一还算空的角落），
并在 PR 正文里主动列明与上述 4-8 条的关系。`contrib/pr-body.md` 目前只写了与 `Luaphes` 那条的关系，
真要提需要按这个定位重写。
